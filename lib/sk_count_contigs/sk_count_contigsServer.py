...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'sk_count_contigs'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from sk_count_contigs.sk_count_contigsImpl import sk_count_contigs
impl_sk_count_contigs = sk_count_contigs(config)
...
async_run_methods = {}
async_check_methods = {}
async_run_methods['sk_count_contigs.count_contigs_async'] = ['sk_count_contigs', 'count_contigs']
async_check_methods['sk_count_contigs.count_contigs_check'] = ['sk_count_contigs', 'count_contigs']
sync_methods['sk_count_contigs.count_contigs'] = True
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'sk_count_contigs'
        ...
        self.rpc_service.add(impl_sk_count_contigs.count_contigs,
                             name='sk_count_contigs.count_contigs',
                             types=[basestring, basestring])
        self.method_authentication['sk_count_contigs.count_contigs'] = 'required'
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "sk_count_contigs but no authentication header was passed"
                            raise err
